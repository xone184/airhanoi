...
const NotificationCenter: React.FC = () => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        setError(null);
        try {
            let url = `${API_BASE}/system/notifications.php?page=${page}&limit=15`;
    ...
    const fetchUnreadCount = useCallback(async () => {
        try {
            const res = await fetch(`${API_BASE}/system/notifications.php?unread=1`, { headers: authHeaders() });
            const data = await res.json();
    ...
    const markAsRead = async (id: number) => {
        setActionLoading(id);
        try {
            await fetch(`${API_BASE}/system/notifications.php?id=${id}`, {
    ...
    const markAllAsRead = async () => {
        setActionLoading(-1);
        try {
            await fetch(`${API_BASE}/system/notifications.php?action=read_all`, {
                method: 'PUT', headers: authHeaders()
            });
