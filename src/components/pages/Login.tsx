...
const Login: React.FC<LoginProps> = ({ onLogin, onSwitchToRegister }) => {
        ...
        const tokenClient = window.google.accounts.oauth2.initTokenClient({
            ...
            callback: async (tokenResponse: any) => {
                ...
                try {
                    ...
                    // Try backend first
                    try {
                        const backendRes = await fetch(`${API_BASE}/auth/auth.php?action=google_login`, {
                            method: 'POST',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${API_BASE}/auth/auth.php?action=login`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.authResponse) {
                ...
                window.FB!.api('/me', { fields: 'name,email,id' }, async (userInfo: any) => {
                    ...
                    try {
                        // Thử gọi backend
                        try {
                            const backendRes = await fetch(`${API_BASE}/auth/auth.php?action=facebook_login`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    email: userInfo.email,
