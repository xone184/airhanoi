    ...
    const fetchData = async () => {
        ...
        try {
            const [ovRes, rankRes, monthRes] = await Promise.all([
                fetch(`${API_BASE}/statistics.php?type=overview&days=${days}`).then(r => r.json()),
                fetch(`${API_BASE}/statistics.php?type=ranking&days=${days}`).then(r => r.json()),
                fetch(`${API_BASE}/statistics.php?type=monthly`).then(r => r.json()),
            ]);
